...
class Clear extends Component {
    ...
    componentDidMount(){
        window.scrollTo(0,0);
        let {arr,cleararr}=this.props;//购物车中的数据 
    ...
    // getdate=()=>{
    //     let {data,cleararr:cleararr1}=this.props;
    //     console.log(cleararr1);
    //     // //请求数据；
    //     // async function fnx16(){
    //     //     let id= data.id;//用户的id；
    //     //     let cleararr=JSON.stringify(cleararr1);//把它变成字符串；
    ...
    render() {
        ...
        let sum=(allprice+yf-yh).toFixed(2);
        let sumSting="￥"+sum;
        console.log(sum);
        return (
        <div id="clearpage">
            ...
            <div className="bottom-pay">
                <a 
                    ...
                    onClick={()=>{
                        clear(listDate);
                        console.log(arr);
                        // init(arr);//这个arr是购物车剩下的东西，但是没有更新
                        // this.getdate();//让后台该数据
                        this.props.history.push("/shopping",null);
                    }}
                >
                    去下单
                </a>
