...
class Content extends Component {
    ...
    render3=()=>{
        ...
        let temp = arr.map((e,i)=>{
            return(
                <p key={i}>
                    {e.checked?<img src={require("../img/对号.png")} alt=""/>:<img src={require("../img/警示.png")} alt=""/>}
                    {e.serve}
                </p>
            )
        })
    ...
    render4=()=>{
        ...
        let {classnum,guigeArr}=this.state;
    ...
    //渲染大图 checkednum
    bigpic=(i)=>{
        let {pathClass,bigpicture}=this.state;
        let {img}=data[pathClass][0];//结构赋值 0是个变量；
