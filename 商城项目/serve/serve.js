...
购物车下单：
        /shoping?act=clear&id=id&dataobj=cleararr     
取消订单：
...
地址添加：
        /address?act=add&id=userid&addressarr=addressarr
...
mongoose.connection.on('connected', (err) => {
        ...
        router.get('/add', async (ctx) => {
                ...
                switch (obj.act) {
                        case "addtocart":
                                ...
                                try {
                                       let newshoppingarr= await Users.update({'id' : id}, {'shopping':shoppingobj})
                                        o.code = 0;
                                        o.msg = "添加购物车成功";
                                        o.data = newshoppingarr;
                                } catch (error) {
                                        o.code = 1;
                                        o.msg = "添加购物车失败";
                                }
        ...
        // 购物车删除数据：del
        // /shoping?act=del&id=userid&dataobj=dataobj  
        router.get('/shopping', async (ctx) => {
                let o = {};
                let obj = ctx.request.query;
                let {id,dataobj:objx}=obj;//用户的id，商品队象
                switch (obj.act) {
                        case "add"://++ --
                                try {
                                ...
                                break;
                        case "del"://删除
                                let delobj =JSON.parse(objx);
                                try {
                                        await Users.update({"id":id},{"$pull":{"shopping":delobj}});
                                        o.code = 0;
                                        o.msg = "删除成功";
                                } catch (error) {
                                        o.code = 1;
                                ...
                                break;
                        case "negation"://全选，取反
                                let shoppingobj =JSON.parse(objx);
                                // console.log(shoppingobj);
                                try {
                                        await Users.update({"id":id,},
                                        {
                                                'shopping':shoppingobj
                                        });
                                        o.code = 0;
                                        o.msg = "checked成功";
                                } catch (error) {
                                ...
                                break;
                        case "checked"://商品的checked
                                let shoppingobj1 =JSON.parse(objx);
                                console.log(shoppingobj1);
                                try {
                                        await Users.update({"id":id,},
                                        {'shopping':shoppingobj1});
                                        o.code = 0;
                                        o.msg = "checked成功";
                                } catch (error) {
                                        o.code = 1;
                                ...
                                break;
                        case "clear"://商品下单结算
                                let cleararr =JSON.parse(objx);
                                console.log(cleararr);
                                try {
                                        await Users.update({"id":id,},{"$push":{"order":cleararr}}
                                        );
                                        // Users.update()
                                        o.code = 0;
                                        o.msg = "下单成功";
        ...
        router.get('/address', async (ctx) => {
                let o = {};
                let obj = ctx.request.query;
                let {id,addressarr}=obj;
                switch (obj.act) {
                        case "add":
                                try {
                                        let addressarrx=JSON.parse(addressarr);
                                        let arr=await Users.update({"id":id},{"address":addressarrx});
                                        console.log(arr);
                                        o.code=1;
                                        o.msg="地址添加成功";
                                } catch (error) {
        ...
        router.get('/order', async (ctx) => {
                let o = {};
                let obj = ctx.request.query;
                let {id,cancelarr} = obj;
                switch (obj.act) {
                        case "cancel":
                                let orderarr =JSON.parse(cancelarr);
                                console.log(orderarr);
                                try {
                                        let a = await Users.findOneAndUpdate({'id' : id},{'order':orderarr})
                                        o.code = 0;
                                        o.msg = "订单操作成功";
                                        o.data = a.order;
                                } catch (error) {
